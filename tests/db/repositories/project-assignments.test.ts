...
describe("project assignments repository", () => {
  ...
  test("self-assigned source is tracked", () => {
    ...
    expect(listSelfAssignedProjectIds(db)).toEqual([
      { memberId: member.id, projectId: project.id },
    ]);
  });
  ...
  test("remove assignment preserves historical record", () => {
    ...
    const assignment = createProjectAssignment(db, {
      memberId: member.id,
      projectId: project.id,
    });
  ...
  test("database rejects duplicate active assignments", () => {
    const member = createMember(db, {
      displayName: "Taro",
      email: "taro@example.com",
      passwordHash: "hash",
