...
メンバーは `/work-logs/month` で勤務時間と配賦時間の差分を解消してから月次工数を提出します。0時間の月も明示提出が必要です。管理者は、当月の活動証跡がある無効メンバーを含む提出対象者について代理提出できます。提出後に日次・週次・月次入力、配賦削除、前日コピー、予定から実績への反映などで実績を変更すると、そのメンバー・月だけが自動的に下書きへ戻ります。予定・キャパシティ・原価だけの変更では戻りません。

管理者は `/period-locks` で対象月をレビュー中にして編集を止め、完全性チェック後に承認できます。月次工数の未提出、勤務時間と配賦時間の不一致、原価スナップショット欠損、活動中の請求対象案件に必要な契約売上・人件費予算の欠損はレビュー開始と承認をブロックします。日次予定と月次予定の差は警告ですが、承認は妨げません。
...
旧バージョンの有効な月次ロックは、アップグレード時に「レビュー中」として移行されます。既存のレビュー中・承認済み月では、当時の提出操作が存在しないため、対象メンバーをシステム移行済み提出として補完します。未締め月は下書きのままです。月次工数提出は実在する `YYYY-MM` と「下書き」「提出済み」の状態だけを保存し、移行時も有効な提出・差し戻し・システム移行済みの記録をそのまま保持します。不正な既存値がある場合、移行は途中状態を残さず失敗するため、値を確認・修正してから再実行してください。

追加される提出テーブルと制約は旧アプリの有効な書き込みと互換性があるため、アプリケーションだけを戻す場合はSQLiteボリュームを維持できます。ただし旧アプリでは提出ゲートや実績変更時の自動差し戻しは働きません。データベーススキーマの移行は前方のみを想定しており、スキーマ自体を戻す必要がある場合は移行前バックアップから復元してください。
