...
旧バージョンの有効な月次ロックは、アップグレード時に「レビュー中」として移行されます。既存のレビュー中・承認済み月では、当時の提出操作が存在しないため、対象メンバーをシステム移行済み提出として補完します。未締め月は下書きのままです。追加される提出テーブルは旧アプリから参照されないため、SQLiteボリュームを維持したロールバックが可能です。ただし旧アプリでは提出ゲートや実績変更時の自動差し戻しは働きません。
