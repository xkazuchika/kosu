  ...
  return db.transaction(
    (transaction) => {
      ...
      if (!close || close.status !== "in_review") {
        throw new Error(
          `${input.month} はレビュー中ではないため承認できません。`,
        );
      }
      ...
      if (completeness.blockers.length > 0) {
        throw new Error(
          `未解決のブロッカーが ${completeness.blockers.length} 件あるため承認できません。`,
        );
      }
      ...
      for (const allocation of listEffortReportRows(tx, {
        endDate: `${input.month}-31`,
      })) {
        if (allocation.allocatedHours > 0)
          activeProjectIds.add(allocation.projectId);
      }

      const rows = listProjectFinancialReview(tx, {
        month: input.month,
      }).filter(
        (row) =>
